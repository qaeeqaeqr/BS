...
def train_IQL_on_pursuit():
    ...
    save_list(rewards, f'./outputs/IQL_pursuit{train_start_time}.pkl')
...
def train_IQL_on_pong():
    ...
    rewards = train_iql(training_env, training_iql,
                        num_episodes=alg_config.num_episodes,
                        seed=alg_config.train_seed,
                        env_name='train IQL in Pong' + train_start_time,
                        save_path=alg_config.IQL_pong_model_path,
                        load_path=None,
                        fig_path=alg_config.output_dir)

    save_list(rewards, f'./outputs/IQL_pong{train_start_time}.pkl')
...
def train_CTDIQL_on_pursuit():
    ...
    rewards = train_ctdiql(training_env, training_ctdiql,
                 num_episodes=alg_config.num_episodes,
                 seed=alg_config.train_seed,
    ...
    save_list(rewards, f'./outputs/CTDIQL_pursuit{train_start_time}.pkl')

def train_CTDIQL_on_pong():
    training_env = cooperative_pong_v5.env(
        ball_speed=pong.ball_speed,
        left_paddle_speed=pong.left_paddle_speed,
        right_paddle_speed=pong.right_paddle_speed,
    ...
    training_ctdiql = CTDIQL(num_agents=2,
                             state_dim=32*21*2,
                             action_dim=pong.n_actions,
                             buffer_size=alg_config.buffer_size,
                             lr=alg_config.lr,
    ...
    rewards = train_ctdiql(training_env, training_ctdiql,
                        num_episodes=alg_config.num_episodes,
                        seed=alg_config.train_seed,
                        env_name='train CTDIQL in Pong' + train_start_time,
                        save_path=alg_config.CTDIQL_pong_model_path,
                        load_path=None,
                        fig_path=alg_config.output_dir)

    save_list(rewards, f'./outputs/CTDIQL_pong{train_start_time}.pkl')
