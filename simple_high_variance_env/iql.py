...
class IQLAgent:
    def __init__(self, state_dim, action_dim, learning_rate=0.01, gamma=0.99, epsilon_max=0.9, epsilon_min=0.2, epsilon_decay=0.999):
...
def train_iql_agents():
    ...
    for episode in range(episodes):
        state1 = tuple(env.state)
        state2 = tuple(env.state2)
        done = [False, False]
        episode_rewards = [0, 0]
        step = 0
        ...
        # 打印训练进度
        if (episode + 1) % 100 == 0:
            print(f"Episode: {episode + 1}, Agent1 Reward: {np.mean(total_rewards[0][-100:]):.2f}, Agent2 Reward: {np.mean(total_rewards[1][-100:]):.2f}")
